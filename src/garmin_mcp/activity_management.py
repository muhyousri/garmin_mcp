...
def register_tools(app):
    ...
    @app.tool()
    async def set_activity_distance(
        activity_id: Union[int, str],
        distance_meters: float,
    ) -> str:
        """Set the distance for an activity.

        Useful for indoor/virtual activities (e.g. Zwift) where the virtual
        distance differs from what the trainer or watch calculated. Overwrites
        the activity's distance in the summary.
        ...
        try:
            ...
            _update_activity_summary(
                activity_id, {"distance": distance_meters}
            )
            ...
            return json.dumps(
                {
                    ...
                    "message": "Activity distance successfully updated",
                },
                indent=2,
            )
        except Exception as e:
            return f"Error updating activity distance: {str(e)}"
